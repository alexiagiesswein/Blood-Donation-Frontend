...
export default function EditDoctor() {
  ...
  const { email, pwd, name, locationId, shiftStart, shiftEnd, id } = doctor;
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    fetch(
      `http://localhost:8080/doctor/edit?email=${email}&password=${pwd}&name=${name}&locationId=${locationId}&shiftStart=${shiftStart}&shiftEnd=${shiftEnd}&id=${id}`,
      {
        method: "PUT",
        headers: {
